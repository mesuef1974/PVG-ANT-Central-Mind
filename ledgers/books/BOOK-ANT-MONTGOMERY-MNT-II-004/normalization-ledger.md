...
Trusted source-grounded tools : TOOL-MONTGOMERY-LARGE-SIEVE-DIAGNOSTIC-001 (C, Ch 19-20) ·
    ...
    TOOL-MONTGOMERY-ADDITIVE-PRIME-CIRCLE-METHOD-DIAGNOSTIC-001 (H, Ch 18, closed — v0.6-h-closure PASS)
...
**Consistency note:** the trusted source-grounded path is C (Ch 19-20) · D (Ch 21) · E bounded gaps (Ch 22, CLOSED v0.6-e-closure PASS; entered as validated_intake). F (Ch 17 prime sums / Type I-II) = CLOSED (v0.6-f-closure PASS; entered as validated_intake). G (Ch 16 Van der Corput support) = CLOSED (v0.6-g-closure PASS); it does NOT revive the quarantined legacy off-diagonal material. H (Ch 18 additive / circle method) = CLOSED (v0.6-h-closure PASS; entered as validated_intake); binary Goldbach stays OPEN and is never claimed. Units A/B and the legacy off-diagonal E are QUARANTINED (cross-volume / source-mismatch); their tools are not trusted. `planned.jsonl` empty. MC-001, MC-002 & MC-005 UNSOLVED. Bounded gaps is a KNOWN theorem, not our result, not twin primes, not a parity break. Book overlay CLOSED per AUDIT-CM-MONTGOMERY-OVERLAY-008 (mastery deferred).
