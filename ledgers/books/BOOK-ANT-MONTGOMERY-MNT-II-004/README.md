# Montgomery MNT-II Frontier Support Layer — Book ANT (v0.6, source-grounding-corrected: trusted C/D/E/F/G closed, quarantined A/B + legacy E)

## Source

```text
Hugh Montgomery (with R. Vaughan lineage), Multiplicative Number Theory II: Primes and Sieves.
Book ID: BOOK-ANT-MONTGOMERY-MNT-II-004.  Status: partial_overlay / source-grounding-corrected (trusted C/D/E/F/G, all closure-reviewed; quarantined A/B + legacy E).
...
**v0.6 — SOURCE-GROUNDING-CORRECTED (Correction 006); trusted closed C/D/E/F/G (five units, all closure-reviewed).**
...
**partial_overlay, Level 2, source-grounding-corrected** — NOT book_overlay_closure, NOT mastery. The rest of the book is unmined (see `missed-treasures.md`). Bounded gaps is a KNOWN theorem — not our result, not twin primes, not a parity breakthrough; BV = average input; EH = conditional. Plan in `v0.6-scope.md` / `v0.6-scope-freeze.md` (superseded banners). **Five trusted units closed (C/D/E/F/G: Ch 16–17 + 19–22). Next = explicit decision: the Ch-18 packet (additive prime number theory — with strict fencing around binary Goldbach) proposed by ChatGPT and permitted by Sufyan. No MNTII-006-H without a packet and permission; no new book. Remaining unmined: Ch 18; appendices E–H support-only.**

**Honest classification:** Diagnostic (book ledger, source-grounding-corrected; trusted C/D/E/F/G closed, quarantined A/B + legacy E). No RH/GRH progress. No complete mastery.
