...
**partial_overlay, Level 2, source-grounding-corrected** — NOT book_overlay_closure, NOT mastery. The rest of the book is unmined (see `missed-treasures.md`). Bounded gaps is a KNOWN theorem — not our result, not twin primes, not a parity breakthrough; BV = average input; EH = conditional. Plan in `v0.6-scope.md` / `v0.6-scope-freeze.md` (superseded banners). **No MNTII-006-F without an explicit Treasure Packet and permission; no new book. Coverage Audit 007 recorded (`audits/montgomery-book-coverage-audit-007.md`): trusted-covered Ch 19–22; unmined candidates Ch 17 (strongest) / Ch 18 / Ch 16; appendices E–H support-only. Next = ChatGPT selects the Treasure Packet candidate.**
