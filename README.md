...
| `ledgers/books/` | مُعدَّنة (treasure overlay مُغلَق): Overholt · Tenenbaum · Mileti · IK · Harman · **Montgomery MNT-II (v0.6، source-grounding-corrected؛ book_overlay_closed وmastery مؤجَّلة: الموثوقُ C/D/E/F/G/H مُغلَقةً · المحجورُ A/B/legacy-E)**؛ + **Opera de Cribro (v0.7 scope-only)** + 7 كتب `available_not_imported` |
...
**Now also (v0.7 — Opera de Cribro scope-only):** بأمرٍ مستقلٍّ (2026-07-10) فُتِح **Opera de Cribro** (Friedlander–Iwaniec، AMS Colloquium 57، 2010؛ هويّةٌ متحقّقة) **scope-only فقط**: مُسجَّلٌ في `books.jsonl`، نطاقُه مُجمَّد (`v0.7-scope.md`/`v0.7-scope-freeze.md`)، عقيدتُه مزروعة — الهدفُ المُعلَن (نظريّةُ شهادةِ الغربال/التكافؤ/التوزيع، لا إتقانٌ ولا حلُّ MC-001) · LB-01..06 مُجمَّدة · أربعُ فئاتِ وحدات · فرضيّاتُ توجيهٍ مؤقّتة · فئاتُ تعديلِ النطاق والديون وفصلُ overlay/objective وسجلُّ `C_sieve` في `governance/scope-amendment-and-debt-policy.md`. **لا تعدينَ ولا وحدة (0 ملفّات OPERA-004-*) ولا تصنيفَ فصلٍ موثوقًا حتى أوّلِ Treasure Packet؛ حالةُ Montgomery لم تُمَسّ.**
...
المسار: `v0.1b` → v0.2 Mileti → v0.3 Tenenbaum-005 → v0.4 IK → v0.5 Harman → treasure retrofit → v0.6 Montgomery → **v0.7 Opera de Cribro (scope-only)**. بلا tag · لا merge إلّا بمراجعة. المواصفةُ الكاملة: `PVG-ANT-Central-Mind-v0.1b-full-spec.md`.
