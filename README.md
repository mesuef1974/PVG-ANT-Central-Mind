...
**Now also (v0.7 — Opera de Cribro، أوّلُ intake):** بأمرٍ مستقلٍّ (2026-07-10) فُتِح **Opera de Cribro** (Friedlander–Iwaniec، AMS Colloquium 57، 2010؛ هويّةٌ متحقّقة) ونُفِّذ أوّلُ intake: **`OPERA-004-A`** (Sifting Sequences as Main-Term/Remainder Certificates) من `OPERA-TREASURE-PACKET-001` (Ch1 §§1.1-1.4) = **validated_intake — NOT closed** (`core_certificate_theory` أساسًا + `reference_integration` ثانويًّا · `packet_mismatch=false` · لا TOOL-ID جديد · ثلاثةُ candidate walls موصوفةٌ غيرُ مُسجَّلة). النطاقُ مُجمَّد والعقيدةُ مزروعة (الهدف · LB-01..06 · فئاتُ الوحدات · تعديلاتُ النطاق/الديون/سجلُّ `C_sieve` في `governance/scope-amendment-and-debt-policy.md`). **الحالة scope_open→partial_overlay؛ لا إغلاقَ في جولة الـintake؛ التالي v0.7-A Closure Review بأمرٍ مستقلّ؛ حالةُ Montgomery لم تُمَسّ.**
