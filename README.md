...
**Now also (v0.7 — Opera de Cribro، أوّلُ intake):** بأمرٍ مستقلٍّ (2026-07-10) فُتِح **Opera de Cribro** (Friedlander–Iwaniec، AMS Colloquium 57، 2010؛ هويّةٌ متحقّقة) ونُفِّذ أوّلُ intake: **`OPERA-004-A`** (Sifting Sequences as Main-Term/Remainder Certificates) من `OPERA-TREASURE-PACKET-001` (Ch1 §§1.1-1.4) = **validated_intake — NOT closed** (`packet_mismatch=true routing_split`: `core_certificate_theory` أساسًا + `reference_integration` ثانويًّا؛ صُحِّح في v0.7-A Attempt 1=RETURN · لا TOOL-ID جديد · **صفرُ WALL جديد** — ثلاثةُ boundaries تبقى Boundaries، لا ربطَ مبكرًا بـWALL-PARITY). النطاقُ مُجمَّد والعقيدةُ مزروعة (الهدف · LB-01..06 · فئاتُ الوحدات · تعديلاتُ النطاق/الديون/سجلُّ `C_sieve` في `governance/scope-amendment-and-debt-policy.md`). **الحالة scope_open→partial_overlay؛ لا إغلاقَ في جولة الـintake؛ التالي v0.7-A Closure Review بأمرٍ مستقلّ؛ حالةُ Montgomery لم تُمَسّ.**
...
المسار: `v0.1b` → v0.2 Mileti → v0.3 Tenenbaum-005 → v0.4 IK → v0.5 Harman → treasure retrofit → v0.6 Montgomery → **v0.7 Opera de Cribro (OPERA-004-A intake)**. بلا tag · لا merge إلّا بمراجعة. المواصفةُ الكاملة: `PVG-ANT-Central-Mind-v0.1b-full-spec.md`.
