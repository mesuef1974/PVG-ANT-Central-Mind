...
**Now also (v0.7 — Opera de Cribro، أوّلُ وحدةٍ مُغلَقة):** بأمرٍ مستقلٍّ فُتِح **Opera de Cribro** (Friedlander–Iwaniec، AMS Colloquium 57، 2010؛ هويّةٌ متحقّقة)، ونُفِّذت ثمّ أُغلقت أوّلُ وحدة: **`OPERA-004-A`** (Sifting Sequences as Main-Term/Remainder Certificates) من `OPERA-TREASURE-PACKET-001` (Ch1 §§1.1-1.4) = **CLOSED، v0.7-A Closure Review PASS** (`AUDIT-CM-V07-A-CLOSURE-001`؛ `packet_mismatch=true routing_split`: `core_certificate_theory` أساسًا + `reference_integration` ثانويًّا؛ **ثلاثُ مراجعاتٍ عدائيّةٍ مستقلّة، الثالثةُ SAFE على a319df4** · 7 بطاقات closure_approved · لا TOOL-ID جديد · **صفرُ WALL جديد** — ثلاثةُ boundaries تبقى Boundaries، لا ربطَ مبكرًا بـWALL-PARITY). النطاقُ مُجمَّد والعقيدةُ مزروعة (الهدف · LB-01..06 · فئاتُ الوحدات · تعديلاتُ النطاق/الديون/سجلُّ `C_sieve` في `governance/scope-amendment-and-debt-policy.md`). **الحالة partial_overlay؛ حالةُ Montgomery لم تُمَسّ.** ثمّ نُفِّذ **intake ثانٍ `OPERA-004-B`** (One-Sided Sieve Certificates and the Prime-Asymptotic Gap؛ `OPERA-TREASURE-PACKET-002`، **Ch5 §§5.2-5.4 + Ch6 §§6.1/6.5 Brun+Fundamental Lemma**) = **validated_intake — NOT closed**، target **LB-03**، `packet_mismatch=true routing_split` (مُسجَّلٌ في كلِّ الطبقات)، بطاقات 008-014 provisional، صفرُ TOOL/WALL؛ **سلّمُ شهاداتٍ ثلاثيّ** `one-sided bound ≠ sifted-set asymptotic ≠ prime-producing asymptotic` (target-purity = Boundary داخل LB-03 لا حكمَ تكافؤ، لا ربطَ بـWALL-PARITY)؛ Selberg مؤجَّل · model-subtraction دَينُ اختبارٍ مفتوح. **التالي v0.7-B Closure Review بحوكمةٍ متوسّطة (مُفنّد واحد + LB-03 semantic check)؛ لا OPERA-004-C.**
...
المسار: `v0.1b` → v0.2 Mileti → v0.3 Tenenbaum-005 → v0.4 IK → v0.5 Harman → treasure retrofit → v0.6 Montgomery → **v0.7 Opera de Cribro (OPERA-004-A CLOSED + OPERA-004-B intake)**. بلا tag · لا merge إلّا بمراجعة. المواصفةُ الكاملة: `PVG-ANT-Central-Mind-v0.1b-full-spec.md`.
