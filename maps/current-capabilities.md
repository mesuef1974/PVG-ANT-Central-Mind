...
1. labeled exact data;
2. labeled structural data;
3. unlabeled geometric shape;
4. scalar observables;
5. population pushforwards;
6. analytic certificates.

### Core objects

The registry includes the full valuation vector, labeled support, Boolean support, labeled and unlabeled height profiles, total height, support size, log mass, divisor box, labeled face incidence, boundary count, margin interior, k-full region, squarefree kernel, residue fiber, character phase, truncated profile, aggregate divisibility data, convolution polytope, and local prime-axis germ.
...
LOSS-1 summary or label loss
LOSS-2 population aggregation loss
LOSS-3 phase/sign loss
LOSS-4 missing analytic certificate
```

Installed laws include:

- prime labels are mathematical data;
- support does not recover heights;
- unlabeled exponent shapes do not recover residues;
- scalar statistics do not recover vectors in general;
- the labeled divisor box recovers the valuation vector, but its cardinality does not;
- full Fourier coefficients are invertible, but Fourier energy is not;
- truncated pointwise data and aggregate sieve data are different levels;
- Bell coefficients recover local germs, but global Euler continuation needs a separate certificate;
- log mass is injective on integer valuation vectors, but it does not preserve additive neighborhood geometry;
- PVG materiality is tested by removing the geometric layer and measuring what capability is lost.

Canonical references:

- `registries/pvg-core-ontology-v1/objects.jsonl`
- `registries/pvg-core-ontology-v1/morphisms.jsonl`
- `maps/pvg-core-grammar-v1.md`
...
1. identify the native PVG object;
2. choose an admissible morphism rather than an analogy;
3. determine whether labels, heights, phase, localization, or population structure are lost;
4. state the strongest valid reverse translation;
5. route the resulting arithmetic object to ANT tools;
