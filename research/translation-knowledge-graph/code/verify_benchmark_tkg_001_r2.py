...
def main() -> None:
    ...
    assert {"definitional", "computational", "multi_hop", "diagnostic", "adversarial", "minimal_pair", "coverage"}.issubset({x["class"] for x in cases})
    ...
    assert report["by_language"]["ar"] == {"passed": 12, "total": 12}
    assert report["by_language"]["en"] == {"passed": 12, "total": 12}
